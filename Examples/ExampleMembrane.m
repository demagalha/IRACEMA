...
K = sparse(K);
M = sparse(M);
[V,O] = eigs(K,M,N_DOF - numel(bc),'sm');
omega = sqrt(diag(O));
% wh{iter} = omega;
% iter
% end
...
for i=1:numel(bc)
	if bc(i) == 1
		V = [zeros(1,sz2); V(1:end,:)];
	else if bc(i) == ndof
		V = [V(1:end,:); zeros(1,sz2)];
	else
		V = [V(1:bc(i)-1,:); zeros(1,sz2); V(bc(i):end,:)];
	end
    end
end
u = cell(size(P));
...
DeformedModel9 = Geometry('surf',Model.pu,Model.U,Model.pv,Model.V,comb);
